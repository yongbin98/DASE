...
def loadxlsxData():
    filename = os.getcwd() + '/DASE/KOBIO_data_final.xlsx'
...
def modelDNN(x_train, x_valid, x_test, y_train, y_valid, y_test):
    ...
    # input = tf.keras.layers.Input(shape=(32,1,1))
    ...
    input = tf.keras.layers.Input(shape=(32,1))
    x = tf.keras.layers.Flatten(name='Flatten')(input)
    ...
    x = tf.keras.layers.Dense(32, activation='relu', name='FC4')(x)
    x = tf.keras.layers.Dense(8, activation='relu', name='FC5')(x)
    output = tf.keras.layers.Dense(1, name='Output', activation = 'relu')(x)
...
def modelXGB(x_train, x_test, y_train, y_test):    
    ...
    # model = xgb.XGBRegressor(objective='reg:squarederror',n_estimators=500, learning_rate=0.005, max_depth=3).fit(x_train,y_train)
    ...
    # pltFeature(model.feature_importances_.flatten(), x_train.columns)
    return test_pred, train_pred
...
def pltFeature(importance, feature_names):
    ...
    # sio.savemat(os.getcwd()+'/DASE/result/single_pred.mat',{'importance' : importance, 'feature_names' : feature_names.values})
    
    ser = pd.Series(importance, index=feature_names)
...
def fold_validation():
    ...
    for i in range(5):
        ...
        ### model Ensemble
        # modelDNN(x_train, x_train, x_test, y_train, y_train, y_test)
        # valid_pred, train_pred = modelXGB(x_train, x_valid, y_train, y_valid)
...
# x_train = train.drop(['DASE'],axis=1);
# x_test = test.drop(['DASE'],axis=1);
# # x_train = x_train.drop(x_train.columns[0:79],axis=1);
# # x_test = x_test.drop(x_test.columns[0:79],axis=1);
# y_train = train['DASE']
# y_test = test['DASE']

# x_train, x_test, y_train, y_test = standarizeInput4(x_train, x_test, y_train, y_test)

# x_train1 = x_train[['GEN', 'AGE', 'BWT', 'HGT', 'BMI', 'CIG_1','CIG_2','CIG_3', 'CIGP', 'CIGY']]
# x_train2 = x_train[['CHYT', 'CIHD', 'CHLP', 'CCHF', 'CARR', 'CPVD','CSTR','COND', 'CRLD', 'COSP', 'CDNC', 'CDWC','CHPE','CHPO', 'COBS', 'CRNF', 'CFEI', 'CPTU'
#                     , 'CLVD', 'CTBC', 'CDPR', 'CPSY', 'CANM', 'CHMA', 'CSMA', 'CMMA', 'HJR', 'HOF']]
# x_train3 = x_train[['PTGA', 'PHGA', 'ESR', 'CRP']]
# x_train4 = x_train[['FBIO', 'BTIME']]
# x_train5 = x_train[['PMTX', 'PHCQ', 'PSSZ', 'PLEF', 'PTAC', 'DMARD_0', 'DMARD_1', 'DMARD_2', 'DMARD_3', 'DMARD_4'
#                     , 'DMARD_5', 'DMARD_6', 'DMARD_7', 'DMARD_8', 'DMARD_9', 'CORT_0', 'CORT_1', 'CORT_2', 'CORT_3', 'CDOSE']]
# x_train6 = x_train[['ESOE_1', 'ESOE_2', 'ESOE_3', 'ESSS_1', 'ESSS_2', 'ESSS_3', 'ESRO_1', 'ESRO_2', 'ESRO_3', 'ECVC_1', 'ECVC_2', 'ECVC_3'
#                     , 'EPLU_1', 'EPLU_2', 'EPLU_3', 'EILD_1', 'EILD_2', 'EILD_3', 'EBOO_1', 'EBOO_2', 'EBOO_3', 'EOBC_1', 'EOBC_2', 'EOBC_3'
#                     , 'EPCD_1', 'EPCD_2', 'EPCD_3', 'EGRN_1', 'EGRN_2', 'EGRN_3', 'EVRN_1', 'EVRN_2', 'EVRN_3', 'EFES_2', 'EFES_3'
#                     , 'EAMY_1', 'EAMY_2', 'EAMY_3', 'EBRO_1', 'EBRO_2', 'EBRO_3', 'EPYO_1', 'EPYO_2', 'EPYO_3']]
# x_train7 = x_train[['RF', 'ACCP']]

# x_test1 = x_test[['GEN', 'AGE', 'BWT', 'HGT', 'BMI', 'CIG_1','CIG_2','CIG_3', 'CIGP', 'CIGY']]
# x_test2 = x_test[['CHYT', 'CIHD', 'CHLP', 'CCHF', 'CARR', 'CPVD','CSTR','COND', 'CRLD', 'COSP', 'CDNC', 'CDWC','CHPE','CHPO', 'COBS', 'CRNF', 'CFEI', 'CPTU'
#                     , 'CLVD', 'CTBC', 'CDPR', 'CPSY', 'CANM', 'CHMA', 'CSMA', 'CMMA', 'HJR', 'HOF']]
# x_test3 = x_test[['PTGA', 'PHGA', 'ESR', 'CRP']]
# x_test4 = x_test[['FBIO', 'BTIME']]
# x_test5 = x_test[['PMTX', 'PHCQ', 'PSSZ', 'PLEF', 'PTAC', 'DMARD_0', 'DMARD_1', 'DMARD_2', 'DMARD_3', 'DMARD_4'
#                     , 'DMARD_5', 'DMARD_6', 'DMARD_7', 'DMARD_8', 'DMARD_9', 'CORT_0', 'CORT_1', 'CORT_2', 'CORT_3', 'CDOSE']]
# x_test6 = x_test[['ESOE_1', 'ESOE_2', 'ESOE_3', 'ESSS_1', 'ESSS_2', 'ESSS_3', 'ESRO_1', 'ESRO_2', 'ESRO_3', 'ECVC_1', 'ECVC_2', 'ECVC_3'
#                     , 'EPLU_1', 'EPLU_2', 'EPLU_3', 'EILD_1', 'EILD_2', 'EILD_3', 'EBOO_1', 'EBOO_2', 'EBOO_3', 'EOBC_1', 'EOBC_2', 'EOBC_3'
#                     , 'EPCD_1', 'EPCD_2', 'EPCD_3', 'EGRN_1', 'EGRN_2', 'EGRN_3', 'EVRN_1', 'EVRN_2', 'EVRN_3', 'EFES_2', 'EFES_3'
#                     , 'EAMY_1', 'EAMY_2', 'EAMY_3', 'EBRO_1', 'EBRO_2', 'EBRO_3', 'EPYO_1', 'EPYO_2', 'EPYO_3']]
# x_test7 = x_test[['RF', 'ACCP']]

# # test_pred1, train_pred1 = modelGridSearch(x_train1, x_test1, y_train, y_test)

# model = xgb.XGBRegressor(n_estimators=300, learning_rate=0.1, subsample=0.7, colsample_bytree=0.9, max_depth=5).fit(x_train1,y_train)
# train_pred1 = model.predict(x_train1.squeeze())
# test_pred1 = model.predict(x_test1.squeeze())

# test_pred2, train_pred2 = modelXGB(x_train2, x_test2, y_train, y_test)
# test_pred3, train_pred3 = modelXGB(x_train3, x_test3, y_train, y_test)
# test_pred4, train_pred4 = modelXGB(x_train4, x_test4, y_train, y_test)
# test_pred5, train_pred5 = modelXGB(x_train5, x_test5, y_train, y_test)
# test_pred6, train_pred6 = modelXGB(x_train6, x_test6, y_train, y_test)
# test_pred7, train_pred7 = modelXGB(x_train7, x_test7, y_train, y_test)

# x_train['DEMO'] = train_pred1
# x_train['COMO'] = train_pred2
# x_train['DACT'] = train_pred3
# x_train['BAGE'] = train_pred4
# x_train['MEDI'] = train_pred5
# x_train['EXFE'] = train_pred6
# x_train['LABF'] = train_pred7

# x_test['DEMO'] = test_pred1
# x_test['COMO'] = test_pred2
# x_test['DACT'] = test_pred3
# x_test['BAGE'] = test_pred4
# x_test['MEDI'] = test_pred5
# x_test['EXFE'] = test_pred6
# x_test['LABF'] = test_pred7

# test_pred, train_pred = modelXGB(x_train,x_test,y_train,y_test)
...
# x_train, x_valid, x_test, y_train, y_valid, y_test = standarizeInput(x_train, x_valid, x_test, y_train, y_valid, y_test)
# x_train, y_train = randomOversampler(x_train, y_train)
# modelGridSearch(x_train, x_test, y_train, y_test)
# modelDNN(x_train, x_train, x_test, y_train, y_train, y_test)
# test_pred, train_pred1 = modelXGB(x_train, x_test, y_train, y_test)
# x_train, y_train = applySMOGN(x_train, y_train)

# filtered_indices1 = y_train[y_train > 4].index
# filtered_indices2 = y_test[y_test > 3].index
...
# y_test[y_test > 7] = 2
# y_test[y_test >= 4] = 1
# test_pred = modelClassification(x_train, x_test, y_train, y_test)
# sio.savemat(os.getcwd()+'/DASE/result/single_pred.mat',{'x_test' : x_test.values, 'y_test' : y_test.values, 'test_pred':test_pred})